//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GTServiceAction.h"

@class GTServicePullRequest, GTServicePullRequestManager;

@interface GTServiceResetComputedPullRequestInformationAction : GTServiceAction
{
    GTServicePullRequest *_servicePullRequest;
    GTServicePullRequestManager *_servicePullRequestManager;
}

+ (id)actionWithServicePullRequest:(id)arg1;
- (void).cxx_destruct;
@property(retain, nonatomic) GTServicePullRequestManager *servicePullRequestManager; // @synthesize servicePullRequestManager=_servicePullRequestManager;
@property(retain, nonatomic) GTServicePullRequest *servicePullRequest; // @synthesize servicePullRequest=_servicePullRequest;
- (void)main;
- (id)initWithServicePullRequest:(id)arg1;

@end

