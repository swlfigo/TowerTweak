//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <FNAppKit/FNView.h>

#import "GTCommitInfoContentViewDelegate-Protocol.h"

@class GTCommitInfoContentView, GTGitCommit, GTStateManager, NSArray, NSScrollView, NSString, NSVisualEffectView;

@interface GTCommitInfoView : FNView <GTCommitInfoContentViewDelegate>
{
    GTGitCommit *_commit;
    NSVisualEffectView *_visualEffectView;
    NSScrollView *_contentScrollView;
    GTCommitInfoContentView *_commitInfoContentView;
    GTStateManager *_viewState;
    NSArray *_customConstraints;
}

- (void).cxx_destruct;
@property(retain, nonatomic) NSArray *customConstraints; // @synthesize customConstraints=_customConstraints;
@property(retain, nonatomic) GTStateManager *viewState; // @synthesize viewState=_viewState;
@property(retain, nonatomic) GTCommitInfoContentView *commitInfoContentView; // @synthesize commitInfoContentView=_commitInfoContentView;
@property(retain, nonatomic) NSScrollView *contentScrollView; // @synthesize contentScrollView=_contentScrollView;
@property(retain, nonatomic) NSVisualEffectView *visualEffectView; // @synthesize visualEffectView=_visualEffectView;
@property(retain, nonatomic) GTGitCommit *commit; // @synthesize commit=_commit;
- (void)commitInfoContentViewDidChangeContentType:(id)arg1;
- (void)updateContentView;
- (BOOL)needsUpdateContentView;
- (void)updateViewComponents;
- (void)updateViewState;
- (void)update;
- (void)viewFrameDidChange:(id)arg1;
- (void)viewDidMoveToWindow;
- (void)updateConstraints;
@property(readonly, nonatomic) NSString *contentTypeKey;
@property(readonly, nonatomic) NSString *revisionKey;
- (void)commonInit;
- (id)initWithCoder:(id)arg1;
- (id)initWithFrame:(struct CGRect)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

